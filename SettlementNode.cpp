...
ASettlementNode::ASettlementNode()
{
	...
	static ConstructorHelpers::FObjectFinder<UUserDefinedStruct> ItemStructure(TEXT("UserDefinedStruct'/Game/Mechanics/items/ItemStructure.ItemStructure'"));
	if (ItemDatabase.Succeeded())
	{
		DataTable = ItemDatabase.Object;
	}
	if (ItemStructure.Succeeded())
	{
		BPStruct = ItemStructure.Object;
	}
	//PrimaryActorTick.bCanEverTick = true;
	//I don't need this to tick every second.
}
...
{
	Super::BeginPlay();
	//The first thing that will happen is the settlement assignment.
	for (size_t i = 0; i < Residents.Num(); i++)
	{
		Residents[i]->Settlement = SettlementName; //Can then be referenced using strings from the database.
	}
	//GetWorldTimerManager().SetTimer(MemberTimerHandle, this, &RecalculateLocalEconemy(), 240.0f, true, 360.0f);
	//Atleast at the start of the game the goods prices should be calculated.
	FoodCoef = FindFoodCoef(GoodsStores,Residents);
...
float ASettlementNode::FindFoodCoef(TArray<AGoodsStore*> StorePlaces, TArray<AHumanoid*> People) //What this does is find the price which food should be sold at. We can assume that to some degree everyone has access to all food and it's really how long the food would last.
{
	...
	if (IsValid(DataTable))
	{
		...
		TArray<FString> FoodTypes;
			...
			for (size_t i = 0; i < Rows.Num(); i++)
			{
				...
				else //Assuming this can't be fixed we'll just have to assume a sixth is food.
				{
					iRChoice = rand() % 6;
					if (iRChoice == 0)
					{
	...
	{
		output = 1; //Just assume prices are at the average/ baseline
	}
